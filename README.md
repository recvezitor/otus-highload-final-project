...
При поверхностном оценке коммуникационных и кол-центров предлагается разделение на 3 тира по количеству
...
ElasticSearch/OpenSearch - не оптимизирован для хранения такого большого объема данных, индексы будут распухать
...
обоснование хайлоадности - где там высокая нагрузка, fault tolerance etc
рассказать про устойчивость к изменениям
показать график таймскейл как он держит нагрузку

про важность тестовой нагрузки - simple, load, scenarios, etalon

начать заполнять презенташку